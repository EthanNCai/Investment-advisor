...
class LSTMPredictor:
    ...
    def _apply_walkforward_validation(self, df: pd.DataFrame, target_col: str, seq_length: int, n_splits: int = 5) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        """
        应用走进式交叉验证，模拟真实的预测场景
    ...
    def predict(self, ratio_data: List[float], dates: List[str], code_a: str, code_b: str,
        ...
        # 确定风险级别
        relative_std = np.mean(std_predictions) / np.mean(np.abs(mean_predictions)) if np.mean(np.abs(mean_predictions)) > 0 else 0
